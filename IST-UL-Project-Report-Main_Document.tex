...
Based on this framework, the first influence diagram was constructed (Fig. \ref{fig:influenceDiagrama}). The model includes a decision node, a chance node, and a value node. The decision node, \emph{Production Strategy}, represents H2People’s choice between independent production, partnership, or selling the patent. The chance node, \emph{Sales}, captures market uncertainty through three possible outcomes: strong, moderate, and weak demand, each with an associated probability.
...
The influence diagram was then converted into a decision tree (Fig. \ref{fig:DecisionTreea}), allowing the calculation of the Expected Monetary Value (EMV) for each strategic alternative.
By comparing the EMVs of the three alternatives, producing the electrolyser independently yields the highest Expected Monetary Value. Therefore, under a risk-neutral assumption, the optimal decision is to produce the electrolyser independently.The influence diagram (Fig. \ref{fig:influenceDiagrama}) includes a decision node (\emph{Production Strategy}), a chance node (\emph{Sales}), and a value node (\emph{NPV}). The decision tree (Fig. \ref{fig:DecisionTreea}) yields EMVs: independent production (309.85 M€), patent sale (180 M€), and partnership (154.93 M€). Under risk neutrality, independent production is optimal.


\section{Question 1b}

To evaluate the value of perfect information, a revised decision tree was analysed assuming that sales outcomes are known before the production decision. In the corresponding influence diagram, an information link from the \emph{Sales} node to the \emph{Production Strategy} node indicates that decisions are taken after market conditions are realised.

Under perfect information, H2People selects for each sales outcome the strategy that maximises the Net Present Value. The Expected Monetary Value with perfect information is therefore obtained as the probability-weighted sum of these optimal conditional payoffs.
...
With perfect information on market outcomes, H2People can select the optimal strategy for each scenario, yielding an EMV of 417.25 M€.
...
With exponential utility function ($R = 10$ M€: $U(x) = 1 - e^{-x/10}$), the certainty equivalent is 181 M€ . The optimal strategy shifts to patent sale regardless of market research, as the utility function penalises the potential 177 M€ loss under weak sales.
