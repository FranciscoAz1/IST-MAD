...
By comparing the EMVs of the three alternatives, producing the electrolyser independently yields the highest Expected Monetary Value. Therefore, under a risk-neutral assumption, the optimal decision is to produce the electrolyser independently.The influence diagram (Fig.~2) includes a decision node (\emph{Production Strategy}), a chance node (\emph{Sales}), and a value node (\emph{NPV}). The decision tree (Fig.~3) yields EMVs: independent production (309.85 M€), patent sale (180 M€), and partnership (154.93 M€). Under risk neutrality, independent production is optimal.


\section{Question 1b}

To evaluate the value of perfect information, a revised decision tree was analysed assuming that sales outcomes are known before the production decision. In the corresponding influence diagram (Fig.~4), an information link from the \emph{Sales} node to the \emph{Production Strategy} node indicates that decisions are taken after market conditions are realised.
...
With perfect information on market outcomes, H2People can select the optimal strategy for each scenario, yielding an EMV of 417.25 M€ (Fig. \ref{fig:decisionTreeb}).
